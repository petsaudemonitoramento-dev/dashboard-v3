...
export default async function AdministrationPage({
  ...
  try {
    ...
    if (usersResult.error || profilesResult.error) {
      throw new Error(
        usersResult.error?.message ?? profilesResult.error?.message,
      );
    }
    ...
    rows = usersResult.data.users.map((user) => ({
      id: user.id,
  ...
  const savedRow = params.salvo
    ? rows.find((row) => row.id === params.salvo)
    : null;
    ...
    <div className="space-y-6">
      ...
      {savedRow && (
        <section className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
          ...
          {savedRow.email} está como <strong>{savedRow.profile?.role}</strong>{" "}
          e {savedRow.profile?.active ? "ativo" : "inativo"}.
        </section>
      )}

      {configurationError ? (
        <section className="panel border-amber-200! bg-amber-50! text-amber-900">
          <h2 className="font-bold">Configuração server-side necessária</h2>
          <p className="mt-2 text-sm">{configurationError}</p>
          <p className="mt-2 text-sm">
            Configure <code>SUPABASE_SECRET_KEY</code> apenas no ambiente do servidor.
        ...
        <section className="panel">
          <div className="panel-heading">
            <span className="eyebrow">{rows.length} usuários autenticados</span>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td colSpan={3}>
                      <form
                        key={`${row.id}:${row.profile?.role ?? "sem-perfil"}:${row.profile?.active ?? false}`}
